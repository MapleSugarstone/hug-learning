...
**Create a curriculum.** Give it a name and a description, then add questions. Each question is either single choice (one correct answer) or multiple choice (one or more correct answers). Mark the correct answers, write an explanation of the correct answer, and optionally add a short note on any incorrect option about why it is incorrect. Tick **Keep the answer options in this order** on a question whose answers depend on their position, such as "All of the above." A blank line in the description starts a new paragraph. Your draft is kept in the browser as you type. When you are done, use **Save** to download the curriculum file. A message then explains the two ways to use it: send the file to Maple to upload to the `curricula` folder so it appears for everyone, or open it yourself from the load screen with **Choose a file** when running the curriculum. **Try it out** runs the draft as a learner would see it. **Open** lists the curricula already in the `curricula` folder so you can edit one, and also lets you choose a saved file from your computer. When you edit an existing curriculum, saving keeps its file name so you can drop it back into the folder. The trash button clears the draft after asking you to confirm.

**Answering.** After each answer the tool shows whether it was right. On a wrong answer the correct options are highlighted, the explanation appears, and any incorrect option the learner picked shows its "why it is incorrect" note. The question is set aside and asked again in the next round. Answer options are shuffled every time a question is shown, including retries, unless the question is marked to keep its order. When every question has been answered correctly the page shows a completion message, asks the learner to wait, and offers a **Rerun learning tool** button that starts the same curriculum over.
...
- `shuffle` on a question, set to `false`, keeps the options in the order written. Otherwise the options are shuffled each time the question is shown.
